...
Variables    ../Locators/ProjectLandingPage_Locator.py
...
Select control unit
    ...
    wait until element is visible    ${select_control_unit_class}    timeout=30
    input text    ${select_control_unit_class}    ${control_unit}
    click link    name=JumpBtn
    #select from list by label    ${select_control_unit_id}      ${control_unit}
    unselect frame

#Select control unit demo
#    [Arguments]    ${control_unit}
#    Select issuedetails frame
...
Input title
    ...
    Select issuedetails frame
    #wait until element is visible    ${input_title_id}    timeout=30
    #input text    id=F11325    ${title_name}
    input text    ${input_title_id}    ${title_name}    True
    unselect frame
